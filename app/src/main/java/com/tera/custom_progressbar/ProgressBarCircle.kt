...
class ProgressBarCircle(
    ...
    init {
        ...
        val maxInt = a.getInt(R.styleable.ProgressBarCircle_max, MAX.toInt())
        mMax = maxInt.toFloat()
        val valueInt = a.getInt(R.styleable.ProgressBarCircle_value, 0)
        mValue = valueInt.toFloat()
    ...
    private fun setNewText(text: String) {
        if (mPercent) {
            if (text.toIntOrNull() != null) {
                if (mMax.toInt() == 0) mMax = MAX
                var percent = (mValue * 100 / mMax).toInt().toString()
                percent = "$percent%"
                mText = percent
            } else mText = text
        } else mText = text
